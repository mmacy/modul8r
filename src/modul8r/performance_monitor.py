...
class SimpleEventLoopMonitor:
    ...
    def __init__(self, max_lag_ms: Optional[float] = None, check_interval: Optional[float] = None):
        ...
        logger.debug("SimpleEventLoopMonitor initialized", max_lag_ms=max_lag_ms, check_interval=check_interval)
    ...
    async def _check_event_loop_lag(self) -> None:
        ...
        # Record measurement
        measurement = {"timestamp": current_time, "lag_ms": lag_ms, "expected_interval": self.check_interval}
        ...
        if lag_ms > self.max_lag_ms:
            logger.warning("Event loop lag detected", lag_ms=round(lag_ms, 2), threshold_ms=self.max_lag_ms)

            # Check for severe lag
            if lag_ms > self.severe_lag_threshold:
                self.severe_lag_count += 1
                logger.error(
                    "Severe event loop lag detected",
    ...
    async def trigger_degradation(self, lag_ms: float) -> None:
        ...
        if not self.degradation_active:
            ...
            logger.warning("Triggering performance degradation", lag_ms=round(lag_ms, 2), degradation_level="standard")
    ...
    async def trigger_emergency_degradation(self, lag_ms: float) -> None:
        ...
        """
        logger.error(
            "Triggering emergency performance degradation",
            lag_ms=round(lag_ms, 2),
            severe_lag_count=self.severe_lag_count,
    ...
    def is_healthy(self) -> bool:
        ...
        return avg_recent_lag <= self.max_lag_ms and not self.degradation_active and self.severe_lag_count == 0
