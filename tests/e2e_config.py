...
class E2EConfig:
    ...
    def validate_profile(self, profile_name: str) -> bool:
        ...
        if not pdf_path.exists():
            raise E2EConfigError(
                f"Profile '{profile_name}' references non-existent PDF: {pdf_path}"
            )
        
        # Validate detail_level
        if profile["detail_level"] not in ["low", "high"]:
            raise E2EConfigError(
                f"Profile '{profile_name}' has invalid detail_level: {profile['detail_level']}"
            )
        ...
        if not isinstance(concurrency, int) or concurrency < 1 or concurrency > 100:
            raise E2EConfigError(
                f"Profile '{profile_name}' has invalid concurrency: {concurrency}"
            )
